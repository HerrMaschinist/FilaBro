...
- Onboarding flow to configure Spoolman server URL
...
- **Navigation**: `expo-router` (file-based routing, similar to Next.js)
  ...
  - `app/spool/[id].tsx` — spool detail screen
...
3. **Repositories** (`src/data/repositories/`): `SpoolRepository`, `FilamentRepository`, `ManufacturerRepository`. Use Drizzle ORM queries against local SQLite. No platform checks — platform differences are handled at the DB client level.

4. **SyncService** (`src/data/sync/SyncService.ts`): Orchestrates pull/push between local DB and Spoolman.
